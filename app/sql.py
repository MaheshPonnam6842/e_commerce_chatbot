...
Do not write anything like 'Based on the data' or any other technical words. 
...
def sql_chain(question):
    ...
    if len(matches)==0:
        return "Sorry LLM is not able to generate query for given question"
    print(f'Sql query: {matches[0].strip()}')
    response= run_query(matches[0].strip())
    if response is None:
        return "There was a problem executing SQL query"
...
if __name__ == "__main__":
    ...
    print(Answer) # output can be different then in flipkart website like(price, rating), data here i trained is depended on data during web scrapping
